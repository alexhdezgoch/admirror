...
export async function analyzePatterns(
  ...
  const { brandId, forceRefresh } = params;
  ...
  if (!forceRefresh) {
    ...
    if (cached) {
      ...
      if (cacheAge < cacheMaxAge) {
        return {
          analysis: cached.analysis as MyPatternAnalysis,
          cached: true,
  ...
  if (!adsData || adsData.length === 0) {
    throw new Error('No ads found. Please sync your Meta ads first.');
  }

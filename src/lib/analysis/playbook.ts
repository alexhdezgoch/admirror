...
export interface GeneratePlaybookResult {
  success: boolean;
  playbook?: GeneratePlaybookResponse['playbook'];
  error?: string;
  details?: GeneratePlaybookResponse['details'];
}
...
export async function generatePlaybook(
  params: {
    ...
    patterns?: MyPatternAnalysis;
  },
  supabase: SupabaseClient,
  userId: string
): Promise<GeneratePlaybookResult> {
  const { brandId, title, trends: preComputedTrends, patterns: preComputedPatterns } = params;
  ...
  let myPatternsData: MyPatternAnalysis | null = preComputedPatterns || null;
  let clientAdsAnalyzed = 0;

  if (!preComputedPatterns) {
    try {
    ...
    clientAdsAnalyzed = preComputedPatterns.adsAnalyzed || 0;
  }
  ...
  let trendsData: DetectedTrend[] = preComputedTrends || [];
  let trendsCount = preComputedTrends?.length || 0;

  if (!preComputedTrends) {
    try {

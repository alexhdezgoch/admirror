...
export async function analyzeTrends(
  params: {
    brandId: string;
    ads: TrendAnalysisRequest['ads'];
    forceRefresh?: boolean;
  },
  supabase: SupabaseClient,
  userId: string
): Promise<AnalyzeTrendsResult> {
  const { brandId, ads, forceRefresh } = params;
  ...
  if (!brandId || !ads || ads.length === 0) {
    throw new Error('brandId and at least one ad are required');
  }
...
`;
  ...
  const uniqueCompetitors = Array.from(new Set(ads.map(ad => ad.competitorName)));
...
Add these fields to each trend object in the JSON response.`;
  ...
  const recentAds = ads.filter(ad => ad.daysActive <= 30);
  ...
  const knownCompetitors = new Set(ads.map(ad => ad.competitorName));
